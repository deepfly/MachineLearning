...
for it in xrange(0, 3000):
	...
	for d in xrange(0, len(data2D)):
		...
		# print sigout
		delta_o = sigout * (1 - sigout) * (label[d]/100 - sigout)
		error = error + (label[d]/100 - sigout) ** 2
		# print delta_o
		delta_h = []
...
for d in xrange(0, len(data2D)):
	...
	output = multiply1D(sigmid, mat2)[0]
	sigout = sigmoid0D(output)
	sys.stdout.write(str(sigout * 100) + "\n")
